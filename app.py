...
_defaults = {
    ...
    "audit_df":              None,
    "gemini_api_key":        "",
    # BIM Fixer — persistencia total entre rerenders
    "fixer_ifc_bytes":       None,
    "fixer_log_df":          None,
    "fixer_exitos":          0,
    "fixer_errores":         0,
    "fixer_tiempo":          0.0,
    "fixer_ejecutado":       False,
    "fixer_total":           0,
    "fixer_ifc_name":        "",
}
    ...
    with col_der:
        if btn_extraer and pdf_file:
            ...
            for idx, frag in enumerate(fragmentos):
                ...
                prompt = f"""
Eres un experto en BIM (Building Information Modeling), IFC e IDS buildingSMART.
Analiza el siguiente fragmento de un BEP/EIR (Plan de Ejecución BIM) y extrae TODOS los requisitos de información de elementos constructivos.

═══════════════════════════════════════════════
REGLAS ABSOLUTAS — DEBES CUMPLIRLAS SIEMPRE:
═══════════════════════════════════════════════

1. FORMATO: Devuelve ÚNICAMENTE un array JSON válido. Sin texto previo, sin markdown, sin bloques ```json.

2. DIFERENCIACIÓN SEMÁNTICA (crítico):
   - ATRIBUTOS DE ENTIDAD → los que existen directamente en el objeto IFC:
     Name, Tag, Description, ObjectType, PredefinedType, LongName
     → usar Estructura: "Attribute"
   - PROPIEDADES DE PSET → todo lo demás (características técnicas):
     IsExternal, LoadBearing, FireRating, ThermalTransmittance, AcousticRating, etc.
     → usar Estructura: "Property"
   NUNCA pongas Name o Tag como Property. NUNCA pongas FireRating como Attribute.

3. REGEX CORRECTO — OBLIGATORIO:
   - Patrones de nomenclatura: usar regex Python válido con .* (no glob *)
   - Ejemplos CORRECTOS:   "MUR-.*"  "VIG-[0-9]+"  "^PIL-"
   - Ejemplos INCORRECTOS: "MUR-*"   "MUR*"         "MUR-?"
   - Para valores booleanos: usar exactamente "True" o "False" (sin comillas extra)
   - Para valores numéricos: usar el número directamente: "60", "0.8", "200"
   - Si no hay restricción específica: dejar Restriccion_Valor como ""

4. TIPO_DATO siempre en MAYÚSCULAS:
   IFCLABEL (texto), IFCBOOLEAN (sí/no), IFCREAL (decimal), IFCINTEGER (entero), IFCTEXT (texto largo)

5. CLASE IFC siempre válida:
   IfcWall, IfcSlab, IfcColumn, IfcBeam, IfcDoor, IfcWindow, IfcStair,
   IfcRoof, IfcCovering, IfcBuildingElementProxy, IfcSpace, IfcZone

6. PropertySet: nombre del Pset donde vive la propiedad.
   Usar estándar: Pset_WallCommon, Pset_SlabCommon, Pset_ColumnCommon, etc.
   Si es un Pset personalizado del proyecto, usarlo textualmente.

═══════════════════════════════════════════
FORMATO JSON — EXACTAMENTE ESTE ESQUEMA:
═══════════════════════════════════════════
[
  {{
    "ID": "REQ-001",
...
# ══════════════════════════════════════════════════════════════════════════════
#  MOTOR DE VALIDACIÓN INTELIGENTE v2.0
# ══════════════════════════════════════════════════════════════════════════════

def normalizar_patron(restriccion: str) -> str:
    """
    Convierte restricciones de texto a regex válido.
    Maneja los patrones que Gemini genera incorrectamente.

    Casos:
      - "MUR-*"   → "MUR-.*"   (glob → regex)
      - "MUR-.*"  → "MUR-.*"   (ya es regex, OK)
      - "TRUE"    → "^(true|false)$" no aplica — se gestiona en validar()
      - ""  / ".*"  → sin restricción de valor
    """
    if not restriccion or restriccion in ("", ".*", "N/A", "None", "nan"):
        return ""

    r = restriccion.strip()

    # Glob a regex: reemplaza * sueltos (no precedidos de .) por .*
    r = re.sub(r'(?<!\.)\*', '.*', r)

    # Asegura que no hay doble punto
    r = r.replace('...', '.*').replace('..', '.*')

    return r


def normalizar_booleano(val: str) -> str:
    """
    Normaliza cualquier representación booleana a 'True' o 'False'.
    Cubre: IFC (.T./.F.), Python (True/False), texto (true/false/1/0).
    """
    v = str(val).strip().lower()
    if v in ("true", ".t.", "1", "yes", "si", "sí", "verdadero"):
        return "True"
    if v in ("false", ".f.", "0", "no", "falso"):
        return "False"
    return val  # no es booleano


def extraer_segmentos_revit(val_str: str) -> list[str]:
    """
    Revit exporta el Name de elementos como:
      'Muro básico:MUR-EXT-01:165589'  →  ['Muro básico', 'MUR-EXT-01', '165589']

    Devuelve todos los segmentos + el string completo para validar contra cualquiera.
    """
    partes = [s.strip() for s in str(val_str).split(":")]
    return partes + [val_str]  # incluye el string completo también


def buscar_valor_en_elemento(el, propiedad: str) -> tuple[str, str]:
    """
    Búsqueda exhaustiva del valor de una propiedad en un elemento IFC.
    Devuelve (valor_encontrado, fuente).

    Estrategia en orden de prioridad:
    1. Atributo directo (Name, Tag, Description, GlobalId…)
    2. PropertySets (búsqueda exacta + case-insensitive)
    3. Alias semánticos (compatibilidad entre versiones IFC y exportadores)
    4. Psets de Revit (Pset_Revit_*, Identity Data, etc.)
    """
    p = propiedad

    # ── 1. Atributo directo ──────────────────────────────────────────────────
    for attr_name in [p, p.lower(), p.capitalize()]:
        if hasattr(el, attr_name):
            v = getattr(el, attr_name)
            if v is not None and str(v).strip() not in ("", "None"):
                return str(v), "atributo"

    # ── 2. PropertySets — búsqueda completa ──────────────────────────────────
    try:
        psets = ifcopenshell.util.element.get_psets(el)
    except Exception:
        psets = {}

    # Búsqueda exacta en todos los Psets
    for pset_name, props in psets.items():
        if p in props and props[p] is not None:
            return str(props[p]), f"Pset:{pset_name}"

    # Búsqueda case-insensitive
    p_lower = p.lower()
    for pset_name, props in psets.items():
        for k, v in props.items():
            if k.lower() == p_lower and v is not None:
                return str(v), f"Pset:{pset_name}[ci]"

    # ── 3. Alias semánticos entre versiones IFC / exportadores ───────────────
    ALIASES = {
        # IFC2x3 ↔ IFC4
        "IsExternal":        ["isExternal", "IsOutside", "External"],
        "LoadBearing":       ["IsStructural", "Structural", "isLoadBearing", "IsLoadBearing"],
        "FireRating":        ["FireResistanceRating", "FireResistance", "EI"],
        "AcousticRating":    ["AcousticPerformance", "SoundInsulation"],
        "ThermalTransmittance": ["UValue", "U-Value", "Uvalue"],
        "OverallWidth":      ["Width", "Thickness", "Ancho"],
        "OverallHeight":     ["Height", "Alto"],
        "OverallDepth":      ["Depth", "Profundidad"],
        "Reference":         ["Type", "TypeName", "ElementType"],
        "Status":            ["Phase", "EstadoElemento"],
        # Revit-specific
        "Name":              ["Family and Type", "Type Name", "Nombre"],
        "Tag":               ["Mark", "Marca", "Número"],
        "Description":       ["Comments", "Comentarios"],
    }

    if p in ALIASES:
        for alias in ALIASES[p]:
            # En atributos
            if hasattr(el, alias) and getattr(el, alias) is not None:
                return str(getattr(el, alias)), f"alias:{alias}"
            # En Psets
            for pset_name, props in psets.items():
                if alias in props and props[alias] is not None:
                    return str(props[alias]), f"alias_pset:{alias}"

    # ── 4. Búsqueda en Psets de Revit (Identity Data, etc.) ─────────────────
    REVIT_PSETS = ["Identity Data", "Datos de identidad", "Pset_Revit_Type_Identity",
                   "Other", "Constraints", "Structural", "Analysis Results"]
    for rp in REVIT_PSETS:
        if rp in psets:
            props = psets[rp]
            if p in props and props[p] is not None:
                return str(props[p]), f"Revit:{rp}"
            for k, v in props.items():
                if k.lower() == p_lower and v is not None:
                    return str(v), f"Revit:{rp}[ci]"

    return "N/A", "no_encontrado"


def validar_cumplimiento(val_str: str, patron_original: str) -> tuple[bool, str]:
    """
    Motor de validación inteligente. Retorna (cumple, motivo).

    Casos cubiertos:
    A. Sin restricción (patron vacío) → PASA si hay valor
    B. Booleano → normaliza ambos lados antes de comparar
    C. Numérico → comparación float con tolerancia
    D. Revit Name (con ':') → valida contra CADA segmento
    E. Regex / glob → búsqueda en string completo (no solo inicio)
    F. Texto exacto → comparación case-insensitive
    """
    # Valores no encontrados siempre fallan
    if val_str in ("N/A", "None", "", "nan"):
        return False, "valor_no_encontrado"

    # A. Sin restricción → basta con que exista el valor
    patron = normalizar_patron(patron_original)
    if not patron:
        return True, "sin_restriccion"

    val_norm    = val_str.strip()
    patron_norm = patron.strip()

    # B. Booleano — normalizar ambos lados
    val_bool    = normalizar_booleano(val_norm)
    patron_bool = normalizar_booleano(patron_norm)
    if val_bool in ("True", "False") or patron_bool in ("True", "False"):
        cumple = val_bool.lower() == patron_bool.lower()
        return cumple, f"bool:{val_bool}=={patron_bool}"

    # C. Numérico con tolerancia del 0.1%
    if es_numero(val_norm) and es_numero(patron_norm):
        v_f, p_f = float(val_norm), float(patron_norm)
        tol    = max(abs(p_f) * 0.001, 1e-9)
        cumple = abs(v_f - p_f) <= tol
        return cumple, f"num:{v_f}≈{p_f}"

    # D. Formato Revit (Name con ':') → valida segmentos individualmente
    segmentos = extraer_segmentos_revit(val_norm)

    # E. Regex sobre cada segmento
    try:
        regex = re.compile(patron_norm, re.IGNORECASE)
        for seg in segmentos:
            if regex.search(seg):
                return True, f"regex_match:{seg}"
        return False, f"regex_no_match:{val_norm}"
    except re.error:
        pass

    # F. Texto exacto case-insensitive sobre cada segmento
    patron_ci = patron_norm.lower()
    for seg in segmentos:
        if seg.lower() == patron_ci or patron_ci in seg.lower():
            return True, f"texto_match:{seg}"

    return False, f"no_match:{val_norm}"
...
def auditar_modelo(reglas: list, ifc_path: str) -> pd.DataFrame:
    """
    Motor de validación IDS vs IFC v2.0
    Usa el sistema de validación inteligente con soporte para:
    - Formato Revit (Name con segmentos separados por ':')
    - Normalización booleana (.T./.F./True/False)
    - Conversión glob→regex automática
    - Búsqueda exhaustiva en atributos + todos los Psets + aliases
    """
    ...
    for r in reglas:
        ...
        for el in elementos:
            ...
            # Búsqueda exhaustiva del valor
            val_str, fuente = buscar_valor_en_elemento(el, p)

            # Validación inteligente
            cumple, motivo = validar_cumplimiento(val_str, r["Restriccion"])

            # Patron limpio para mostrar (sin ^ $ técnicos)
            patron_display = normalizar_patron(r["Restriccion"]) or r["Restriccion"]

            # GUIDs en ambos formatos
            guid_22 = el.GlobalId
            try:
...
if modulo_activo == "Auditoría":
    ...
    with col_der:
        ...
        if st.session_state.audit_df is not None:
            ...
            # Columnas a mostrar (ocultar columnas técnicas por defecto)
            cols_display = ["GUID", "Clase", "Parámetro", "Requisito", "Modelo", "Estado"]
            mostrar_debug = st.toggle("Mostrar columnas de diagnóstico (Fuente / Motivo)", value=False)
            if mostrar_debug and "Fuente" in df_show.columns:
                cols_display = ["GUID", "Clase", "Parámetro", "Requisito", "Modelo", "Fuente", "Motivo", "Estado"]

            # Resumen de fallos por parámetro
            if filtro_estado in ("Todos", "Solo fallos"):
                fallos_group = df[df["Estado"].str.contains("FALLA", na=False)].groupby("Parámetro").size().reset_index(name="Nº Fallos")
                if not fallos_group.empty:
                    with st.expander(f"📊 Resumen de fallos por parámetro ({len(fallos_group)} parámetros con incumplimientos)"):
                        st.dataframe(fallos_group.sort_values("Nº Fallos", ascending=False), use_container_width=True, hide_index=True)
...
if modulo_activo == "BIM Fixer":
    ...
    with col_izq:
        ...
        if ifc_fix:
            st.markdown(f'<div class="bim-card bim-card-accent-green"><span class="mono" style="color:#10b981;font-size:0.75rem">IFC: {ifc_fix.name}</span> <span style="color:#3a4a60;font-size:0.7rem">{ifc_fix.size/1024/1024:.2f} MB</span></div>', unsafe_allow_html=True)

        if df_audit_fix is not None:
            col_est_tmp = next((c for c in df_audit_fix.columns if "estado" in c.lower()), None)
            if col_est_tmp:
                n_fallos = len(df_audit_fix[df_audit_fix[col_est_tmp].str.contains("FALLA", na=False)])
                st.markdown(f'<div class="bim-card bim-card-accent-red"><span style="color:#ef4444;font-size:0.85rem;font-weight:600">{n_fallos}</span> <span style="color:#5a6a85;font-size:0.75rem">elementos a corregir</span></div>', unsafe_allow_html=True)
        ...
        if st.session_state.fixer_ejecutado:
            if st.button("↺ Nueva corrección", key="reset_fixer"):
                for k in ["fixer_ejecutado","fixer_ifc_bytes","fixer_log_df","_fix_df_fallos","_fix_col_guid","_fix_col_clase","_fix_col_param","_fix_col_req"]:
                    if k in st.session_state:
                        del st.session_state[k]
                st.session_state.fixer_ejecutado = False
                st.rerun()

    with col_der:

        # FASE 1: Preparar tabla editable
        if btn_fix and ifc_fix and df_audit_fix is not None:
            ...
            col_guid   = next((c for c in df_audit_fix.columns if c.lower() == "guid"), df_audit_fix.columns[0])
            col_clase  = next((c for c in df_audit_fix.columns if "clase" in c.lower()), None)
            col_param  = next((c for c in df_audit_fix.columns if "par" in c.lower()), None)
            ...
            if not all([col_estado, col_clase, col_param, col_req]):
                ...
                if df_fallos.empty:
                    st.success("El modelo ya cumple todos los requisitos.")
                else:
                    df_fallos["Valor_Correccion"] = df_fallos[col_req].astype(str)
                    st.session_state["_fix_col_guid"]  = col_guid
                    st.session_state["_fix_col_clase"] = col_clase
                    st.session_state["_fix_col_param"] = col_param
                    st.session_state["_fix_col_req"]   = col_req
                    st.session_state["_fix_df_fallos"] = df_fallos

        # FASE 2: Mostrar tabla editable (persiste entre rerenders)
        if "_fix_df_fallos" in st.session_state and not st.session_state.fixer_ejecutado:
            df_fallos  = st.session_state["_fix_df_fallos"]
            col_guid   = st.session_state["_fix_col_guid"]
            col_clase  = st.session_state["_fix_col_clase"]
            col_param  = st.session_state["_fix_col_param"]
            col_req    = st.session_state["_fix_col_req"]

            st.markdown('<div class="section-tag">Revisión de valores a inyectar</div>', unsafe_allow_html=True)
            st.caption("Edita 'Valor_Correccion' si el Requisito es un patrón RegEx.")

            cols_show = [col_guid, col_clase, col_param, col_req, "Valor_Correccion"]
            df_edit = st.data_editor(
                df_fallos[cols_show],
                column_config={
                    col_guid:  st.column_config.TextColumn("GUID", width="medium"),
                    col_clase: st.column_config.TextColumn("Clase", width="small"),
                    col_param: st.column_config.TextColumn("Parámetro", width="small"),
                    col_req:   st.column_config.TextColumn("Requisito", width="small"),
                    "Valor_Correccion": st.column_config.TextColumn("Valor a inyectar", width="medium"),
                },
                use_container_width=True,
                hide_index=True,
                num_rows="fixed",
            ...
            if st.button("Confirmar y Ejecutar", type="primary"):
                ...
                for i, (_, row) in enumerate(df_edit.iterrows()):
                    ...
                    valor = str(row["Valor_Correccion"]).strip()
                    ...
                    if ok:
                        ...
                        errores += 1
                        log_lines_fix.append(f'<span class="log-err">[{i+1}/{total_fix}] {guid[:14]}… | {msg}</span>')
                ...
                with open(out_path, "rb") as f_ifc:
                    st.session_state.fixer_ifc_bytes = f_ifc.read()
                try:
                    os.unlink(tmp_path)
                    os.unlink(out_path)
                except Exception:
                    pass
                st.session_state.fixer_log_df    = pd.DataFrame(log_resultados)
                st.session_state.fixer_exitos    = exitos
                st.session_state.fixer_errores   = errores
                st.session_state.fixer_tiempo    = t_total
                st.session_state.fixer_total     = total_fix
                st.session_state.fixer_ifc_name  = ifc_fix.name if ifc_fix else "modelo"
                st.session_state.fixer_ejecutado = True
                st.rerun()

        # FASE 3: Mostrar resultados persistidos con botones de descarga
        if st.session_state.fixer_ejecutado and st.session_state.fixer_ifc_bytes is not None:
            exitos    = st.session_state.fixer_exitos
            errores   = st.session_state.fixer_errores
            t_total   = st.session_state.fixer_tiempo
            total_fix = st.session_state.fixer_total
            df_log    = st.session_state.fixer_log_df

            st.markdown("""
            <div class="bim-card bim-card-accent-green" style="margin-bottom:14px">
            <span class="mono" style="color:#10b981;font-size:0.82rem;font-weight:600">
            Correccion completada — IFC listo para descargar
            </span>
            </div>
            """, unsafe_allow_html=True)
            ...
            r3.markdown(f'<div class="metric-box"><div class="metric-val mv-amber">{t_total}s</div><div class="metric-lbl">Tiempo</div></div>', unsafe_allow_html=True)

            st.markdown("---")

            d1, d2 = st.columns(2)
            with d1:
                nombre_out = st.session_state.fixer_ifc_name.replace(".ifc","") + "_CORREGIDO.ifc"
                st.download_button(
                    label="⬇ Descargar IFC Corregido",
                    data=st.session_state.fixer_ifc_bytes,
        ...
        elif not st.session_state.fixer_ejecutado and "_fix_df_fallos" not in st.session_state:
            st.markdown("""
            <div class="bim-card" style="text-align:center; padding:3rem 1rem; color:#2a3a50">
                <div style="font-size:2rem; margin-bottom:8px">⬡</div>
